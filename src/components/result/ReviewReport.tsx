...
/** 한 문항 리뷰 카드 — 정답/오답 모두. 오답이면 내 답 줄을 함께 보여준다. */
function RecordCard({ r, i }: { r: PracticeRecord; i: number }) {
    ...
    >
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="inline-flex items-center rounded-full bg-indigo-500 px-2.5 py-0.5 text-[10.5px] font-semibold text-white">
          {r.part === 7
            ? QUESTION_TYPE_META[normalizeCategory(r.question.category)].label
        ...
        {r.isCorrect && (
          <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-0.5 text-[10.5px] font-semibold text-emerald-600 ring-1 ring-emerald-500/10">
            ✓ 정답
          </span>
        )}
      </div>
      ...
      <p className="mt-0.5 text-[13px] text-neutral-400">{r.question.promptKo}</p>
...
export default function ReviewReport({
  ...
  // Part 6·7은 지문 기반 → 다 맞혀도 지문 해설을 복습으로 남긴다.
  const hasPassage = history.some((r) => r.part === 6 || r.part === 7);

  if (wrong.length === 0) {
    // 다 맞힌 지문형(6·7): 축하 + 지문/해설 리뷰
    if (hasPassage) {
      return (
        <div className="space-y-3">
          <div className="card relative overflow-hidden px-6 py-6 text-center">
            <span className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-emerald-400 to-teal-400" />
            <span className="pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-emerald-50/60 to-transparent" />
            <div className="relative">
              <div className="text-3xl">🎉</div>
              <p className="mt-2 font-bold text-neutral-900">전부 맞혔어요</p>
              <p className="mt-1 text-[14px] text-neutral-500">
                지문 해설로 한 번 더 다지고 넘어가세요.
              </p>
            </div>
          </div>
          <p className="label px-1">지문 해설</p>
          {history.map((r, i) => (
            <RecordCard key={r.question.id + i} r={r} i={i} />
          ))}
        </div>
      );
    }
    // 그 외(Part 5 등 지문 없음): 간단한 축하
    return (
