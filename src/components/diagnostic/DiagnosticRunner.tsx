...
function buildBlocks(test: DiagnosticTest): Block[] {
  const blocks: Block[] = [];
...
function blockPartLabel(b: Block): string {
  if (b.kind === "p2") return "Part 2";
  if (b.kind === "lcset") return `Part ${b.set.part}`;
  if (b.kind === "p5") return "Part 5";
  return `Part ${b.set.part ?? 7}`;
}

export default function DiagnosticRunner({ test }: { test: DiagnosticTest }) {
  const router = useRouter();
  const startPractice = usePracticeStore((s) => s.start);
  const blocks = useMemo(() => buildBlocks(test), [test]);
  ...
  const pick = (qid: string, ci: number) =>
    setAnswers((prev) => ({ ...prev, [qid]: ci }));

  // 추천 학습 시작: LC는 리스닝 목록(필터), RC는 연습 스토어 구동 후 /game
  ...
  const submit = () => {
    const r = scoreDiagnostic(test, answers);
    setResult(r);
  ...
  if (!blocks.length) {
    return (
      <main className="container-narrow flex min-h-dvh items-center justify-center py-10">
        <p className="text-neutral-500">진단 콘텐츠를 불러오지 못했습니다.</p>
      </main>
    );
  }
    ...
    return (
      <main className="container-narrow relative min-h-dvh overflow-hidden py-8 pb-safe sm:py-14">
        ...
        >
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-violet-500 ring-1 ring-violet-500/15 backdrop-blur-sm">
            <span className="h-1.5 w-1.5 animate-glow-pulse rounded-full bg-violet-500" />
            LEVEL DIAGNOSTIC
          </span>
          ...
          <p className="mt-4 max-w-md text-[15px] leading-relaxed text-neutral-500">
            실제 토익과 유사한 미니 모의고사입니다. 리스닝(Part 2·3·4) 12문항과 리딩(Part 5·6·7)
            18문항을 풀면 <b className="text-neutral-700">LC·RC 예상 점수</b>와 취약 파트를 진단해 드려요.
          </p>

          <div className="mt-7 grid grid-cols-3 gap-2.5">
            <IntroStat value="30" label="문항" />
            <IntroStat value="약 15분" label="소요" />
          ...
          <div className="mt-6 space-y-2.5 rounded-2xl bg-white/70 p-4 ring-1 ring-neutral-900/[0.05] backdrop-blur-sm">
            ...
            <Tip icon="📝" text="실전처럼 전 문항을 푼 뒤 한 번에 채점됩니다." />
            <Tip icon="📊" text="결과에서 파트별 정답률과 추천 학습을 확인하세요." />
          </div>

          <button
            type="button"
            onClick={() => setPhase("test")}
            className="mt-7 min-h-[54px] w-full rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 text-[16px] font-bold text-white shadow-lg shadow-violet-500/20 transition active:scale-[0.98]"
          >
            진단 시작하기 →
          </button>
        </motion.div>
      ...
      <div className="sticky top-0 z-20 -mx-4 mb-5 bg-gradient-to-b from-white via-white/95 to-white/70 px-4 py-3 backdrop-blur-sm sm:-mx-6 sm:px-6">
        <div className="flex items-center justify-between text-[12px] font-semibold">
          <span className="text-violet-600">
            {blockPartLabel(block)} · {domainOfPart(block.kind === "p2" || block.kind === "lcset" ? (block.kind === "p2" ? 2 : block.set.part) : block.kind === "p5" ? 5 : block.set.part ?? 7) === "LC" ? "리스닝" : "리딩"}
          </span>
              ...
              <button
                ...
                className={`min-h-[64px] rounded-2xl text-[20px] font-extrabold transition active:scale-95 ${
                  sel
                    ...
                    : "bg-white text-neutral-400 ring-1 ring-neutral-200 hover:ring-cyan-300 hover:text-cyan-600"
                }`}
              >
                {L}
              </button>
...
function ChoiceCard({
  ...
  const ring = accent === "cyan" ? "ring-cyan-300 text-cyan-600" : "ring-indigo-300 text-indigo-600";
  const solid =
    accent === "cyan"
      ? "from-cyan-500 to-sky-500"
      : "from-indigo-500 to-violet-500";
  const badge = accent === "cyan" ? "bg-cyan-500" : "bg-indigo-500";
  return (
            ...
            <button
              ...
              className={`flex w-full items-start gap-2.5 rounded-xl border px-3 py-2.5 text-left transition active:scale-[0.99] ${
                sel
                  ...
                  : `border-neutral-200 bg-white hover:${ring.split(" ")[0]}`
              }`}
            >
...
function AudioClip({ clipId, label }: { clipId: string; label: string }) {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-cyan-500 to-sky-600 p-4 text-white shadow-md">
      <p className="text-[12px] font-medium text-white/80">🎧 {label}</p>
      <div className="mt-3 flex items-center gap-3">
        <button
          type="button"
          onClick={toggle}
          className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-white text-cyan-600 shadow transition active:scale-90"
        ...
        <div className="min-w-0 flex-1">
          <div className="h-1.5 overflow-hidden rounded-full bg-white/25">
            <div className={`h-full rounded-full bg-white/80 ${playing ? "animate-pulse" : ""}`} style={{ width: playing ? "100%" : "0%", transition: "width 0.3s" }} />
          </div>
  ...
  return (
    <main className="container-narrow relative min-h-dvh overflow-hidden py-8 pb-safe sm:py-12">
      ...
      >
        ...
        <span className="mt-3 inline-block rounded-full bg-white/10 px-4 py-1.5 text-[13px] font-semibold text-white ring-1 ring-white/15">
          {result.levelLabel}
        </span>
      </motion.div>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-neutral-100">
                  <motion.div
                    ...
                    className={`h-full rounded-full bg-gradient-to-r ${
                      lc ? "from-cyan-400 to-sky-500" : "from-indigo-400 to-violet-500"
                    }`}
                  />
                </div>
              </div>
      ...
      {/* 추천 학습 */}
      <div className="mt-4 rounded-2xl bg-gradient-to-br from-violet-50 to-white p-5 ring-1 ring-violet-500/10">
          ...
          {weakest.map((p) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl bg-white px-4 py-3 text-left ring-1 ring-neutral-900/[0.05] transition hover:ring-violet-300 active:scale-[0.99]"
              >
                <span>
      ...
      <div className="mt-6 flex flex-col gap-2.5">
        <button
          type="button"
          onClick={onHome}
          className="min-h-[52px] w-full rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 text-[15px] font-bold text-white shadow-md transition active:scale-[0.98]"
        >
          학습하러 가기
        </button>
        <button
      ...
      <p className="mt-4 text-center text-[11px] leading-relaxed text-neutral-400">
        ※ 예상 점수는 30문항 표본을 990점 척도로 환산한 추정치입니다. 실제 시험 점수와 다를 수 있어요.
      </p>
    </main>
  );
}
...
function ReviewBlock({
  ...
  const questions =
    block.kind === "lcset" ? block.set.questions ?? [] : block.set.questions;
  return (
    <>
      {questions.map((q: any) => (
        <ReviewCard
...
function Aura() {
  return (
    <>
      <div className="pointer-events-none absolute -right-16 -top-10 -z-10 h-80 w-80 rounded-full bg-gradient-to-br from-violet-400/25 to-fuchsia-400/15 blur-[80px]" />
...
function IntroStat({ value, label }: { value: string; label: string }) {
  return (
    <div className="rounded-2xl bg-white/70 px-3 py-3 text-center ring-1 ring-neutral-900/[0.05] backdrop-blur-sm">
      <p className="text-[17px] font-extrabold text-neutral-900">{value}</p>
