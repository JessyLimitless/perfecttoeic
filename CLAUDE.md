...
- **검증**: `tsc --noEmit` 통과. dev에서 30세트 전부 `/listening/<id>` 200·`/listening/bogus` 404·오디오 mp3 200·`/api/listening`=30세트 확인. (중간에 `.next` 워커 크래시 "Jest worker exceeding retry limit"=500 → node kill+rm .next+단일 재기동으로 정상화 [[toeicnet-dev-server]].)
