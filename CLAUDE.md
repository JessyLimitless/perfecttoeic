...
### 세션 로그 — 2026-07-18 (37차: 패턴학습 인쇄용 책자 UI 전면 업그레이드 — 시맨틱 HTML 조판) ✅ 완료·커밋
- **맥락**: 직전 세션에서 토큰 한계로 중단된 **"책자 UI 너무 구려서 업그레이드 중"** WIP를 이어받음. git status에 미커밋 `mock.ts`·`mock-loader.ts`(보류건)와, scripts에 개념증명 잔여물(`sample-pattern-html.mjs`·`.sample-*.{png,html,mjs}`·`패턴학습_샘플_new.pdf`)이 있었음. 트랜스크립트/파일 확인 결과, 구버전 산문 MD 책자(`패턴학습_책자.md` + marked + 외부 LinkMD CSS)의 저품질 룩을 버리고 **content JSON에서 직접 시맨틱 HTML로 조판**하는 방식으로 전환 중이었고, **패턴 2개(p5-pat-01·p7-pat-01)만 렌더한 샘플**까지 만든 상태였음.
- **신규 생성기 `scripts/build-pattern-html.mjs`** (샘플을 전체 75패턴 책자로 승격): `content/patterns/*.md`(JSON)에서 직접 조판 → 인쇄용 PDF(puppeteer-core, Chrome print). **표지 + 파트 커버 3종(챕터 목차 포함) + 챕터 간지 15종 + 패턴 75 + 예제 260**.
  - **P5**=🎯핵심공식·💡3초컷팁 콜아웃 / **P6·7**=🧭지문흐름예측맵 + 지문 패널.
  - **지문 3분기 렌더**: ① 메타헤더(From/To/Date/Subject) 전폭 헤더 + 2단 본문(24건) ② 표형(송장·일정표, `|`/`-----` 감지) 고정폭 `pre-wrap`(7건, 표 정렬 유지 + 긴 산문줄만 줄바꿈 — 초안 `white-space:pre`가 NOTES 긴 줄 우측 클리핑하던 것 수정) ③ 일반 산문 2단. 빈칸 `(1)`·`____`·`----` 강조.
  - **예제**: 보기 2열 그리드(정답=초록) + 정답 카드(정답 배지+글자+선택지 헤더 → 근거 문단 + ✓/✗ 보기분석(정답 굵게) + 해석 번역 박스). 정답 헤더 CSS 클래스(ans-lab/ans-ltr/ans-choice) 실제 사용에 맞게 보정.
  - **구조적 우월**: 외부 LinkMD(`C:/Users/.../LinkMD`) 의존 제거 + **소스 JSON 직접 조판이라 stale 없음**(정답키 by construction, 별도 동기화 스크립트 불필요).
  - `--sample` 모드(p5·p7 각 1패턴 빠른 미리보기) 유지.
- **폐기(사용자 확정)**: 구버전 파이프라인 `scripts/build-pattern-book.mjs`·`scripts/build-pattern-pdf.mjs`·산문 `패턴학습_책자.md` 삭제(git rm). PoC 잔여물도 정리. `.gitignore` 갱신(구 스크립트 참조→html, `패턴학습_샘플_new.pdf`·`scripts/.book-preview.html` 추가). 정식 PDF `패턴학습_책자.pdf`(gitignore·재생성용)가 이제 새 디자인.
- **검증**: 미리보기 HTML을 puppeteer로 캡처해 **육안 확인**(표지·P6 커버+챕터목차·P5 공식/팁 패턴·P6 이메일 지문+빈칸·P7 송장 표형 지문 — 전부 프리미엄 톤). 구조 카운트 **파트커버 3 · 챕터간지 15 · 패턴 75 · 예제 260 전부 일치**. PDF 6.0MB 생성.
- **커밋 `8937a39`** (책자만, mock WIP 제외). **배포 불필요**(책자는 공부용 산출물, 웹앱 무관 [[pattern-booklet-pdf]]).
- **후속(라이브 프리뷰 서버) ✅ 커밋 `a02fb80`**: 사용자 지적 "편집→재생성→재저장 마찰이라 문서편집기 같지 않다" → 라이브 리로드로 해결. 조판 로직을 공유 모듈 `scripts/pattern-book-render.mjs`(`buildBookHtml(sample)` export, 부수효과 없음)로 분리하고, 생성기는 그걸 import(PDF 바이트 동일 회귀 확인). 신규 `scripts/pattern-book-serve.mjs` = `node scripts/pattern-book-serve.mjs`(localhost:5177) → `content/patterns/*.md` 저장 시 fs.watch(디바운스)+SSE로 **브라우저 자동 새로고침**, 매 요청 소스 재조판(항상 최신), JSON 편집 중 깨지면 **에러 화면**(크래시 X·복원 시 자동 정상화), 우측 상단 🖨 인쇄 버튼(인쇄 시 숨김). 검증: SSE reload 발동·파손→에러화면→복원 정상·PDF 회귀 없음. **자립형 `패턴학습_책자.html`**(외부 리소스 0, 더블클릭 인쇄용)도 생성(gitignore).
- **⏸️ 보류 유지**: 모의고사(mock) WIP 여전히 미커밋(`mock.ts`·`mock-loader.ts`·`content/mock/`).
- **다음 후보**: 책자에 목차(TOC) 페이지·색인 추가 / 모의고사 RC 보강·커밋 / 패턴학습 진도 로비 노출 / PatternStudy UX 폴리시.

### 세션 로그 — 2026-07-23 (38차: 학습 엔진 전환 — 간격반복 + 유형별 약점 리포트 + 디자인 통일) ✅ 완료·배포됨
- **맥락**: 사용자가 유료화를 검토하며 서비스 평가를 요청 → 제품 진단 결과 **"정복 = 진도 체크일 뿐 학습이 아니다"**가 최대 결함으로 나옴(맞힌 문항을 영구 제외 → 망각곡선상 다시 틀릴 문항을 재회할 기회 없음). 사용자가 "1·2번 지금 할 수 있겠는데?"로 착수 확정. 이후 로그인·결제는 보류하고 **"유저는 나 혼자"** 전제로 서비스 완성도에 집중하기로.
- **① 간격반복(SRS) — 신규 `src/game/review.ts`**: 문항별 Leitner 카드(`box/due/seen/correct/last/cat`), 간격 **1→3→7→16→35일**. `store.ts`의 `dropSolved`(맞힌 문항 영구 제외) → **`dropSettled`**(안 푼·틀린·**복습일 도래**)로 교체. `practiceReview()` 복습 전용 드릴 추가.
  - **작업 중 발견한 결함**: 풀에 포함만 시키면 Part 5 문항 1,270개 사이에 복습 몇 개가 묻혀 영영 안 나옴 → **`cycleFillDueFirst`**(복습 든 세트를 큐 앞으로) + **`hoistDue`**(Part 5는 세트 내부에서도 앞으로, 6·7은 지문 순서라 불변).
  - 대결(속도전)은 `coverageOnly` — 노출·정답 수만 누적하고 **스케줄 불변**(시간압박 실수로 간격이 깨지거나 찍어 맞힌 게 장기기억으로 승격되면 안 됨).
  - 복습에서 틀리면 mastery streak도 0 → **정복도가 실제 기억을 반영**하는 정직한 숫자가 됨.
- **② 유형별 약점 리포트 — 신규 `src/game/skills.ts` + `/report`**: Part 5·6 문법 라벨 40여 종을 **11개 문법 스킬**로 묶고 Part 7 6유형 + LC 8유형과 합쳐 **25개 통합 축**(콘텐츠 원문 category 무수정, 런타임 매핑). 집계 소스는 review 카드에 저장해 둔 `cat` → **문제은행 재로드 불필요**. 화면: 오늘의 복습 → 약점 TOP 3 → 유형 전체표, 각 행 원탭 집중 드릴(`practiceFocus`에 `categories[]`·`sets` 인수 추가). 정답률 **85% 이상은 약점으로 부르지 않음**, 표본 **5문항 미만은 순위 제외**(오진 방지). 랜딩에 `ReviewNudge` 스트립(기록 없으면 미노출).
  - 배선: 기록 지점 4곳(store.end / ListeningPlayer 2곳 / 대결 2곳)에 `recordReviews` 부착. `LcBattleItem`에 `category` 추가.
- **③ 디자인 통일**: 측정 결과 주요 6화면이 **18개 색 계열** 사용(회색 neutral 749 vs slate 39 이중화, 파랑 cyan·sky·teal·blue·indigo 5종 경쟁). **`globals.css` 상단에 「팔레트 계약」 명문화**(회색=neutral · 브랜드/RC=indigo→violet · LC=cyan · 정복=emerald · 약점=amber · 오답=rose · 복습=violet→fuchsia, 몸풀기 모드색·정복 등급 램프는 의도된 예외). slate→neutral(33곳), 리스닝 파트 tint 파랑 4종→cyan 명도 램프, 패턴 블롭→브랜드, yellow→amber, red→rose.
  - **잠재 버그 수정**: tailwind safelist에 6개 클래스 누락 → **정복 등급 배지(챌린저·컨텐더·프로·그랜드마스터) 그라데이션이 purge되고 있었음**.
  - **랜딩 서사 버그**: `journeyHint`가 엄격한 순차라 게임으로 290문제를 푼 사람에게도 매번 "먼저 패턴을 익히세요" → 뒷 단계에 진도가 있으면 **명령이 아니라 제안**으로. 3박스 `pb-24`가 복습 스트립을 6rem 밀어내던 것 수정, 데스크탑 lg에서 세로 가운데 정렬, BGM 위젯 모바일 축소(아이콘 pill)+`pb-safe` 여유+"일시정지"→"정지됨".
- **검증**: 신규 `scripts/test-review.mjs` **25/25**(스케줄·리셋·속도전 보호·유형 매핑·표본 부족). 브라우저 E2E — **정복 완료로 심은 문항이 복습일 경과 후 1,270문항 중 첫 문제로 실제 출제됨** / 리포트 렌더 / 드릴 진입 시 해당 유형만 출제. `tsc` 0 + 프로덕션 build 통과. 데스크탑·모바일 스크린샷 육안.
- **배포**: `a6d77e7`(엔진) → `d46f20b`(디자인) push → EC2 수동 재배포 2회([[cloud5-manual-redeploy]], 대시보드 건너뛰고 SSH). 실서버 `/`·`/report`·`/rank`·`/listening`·`/game`·`/learn`·`/patterns`·`/conquest/5` 전부 **200**. rollback `:prev` 확보, 임시 키 삭제.
- **유료화 관련 조언(요청)**: 상용화 관문 = ① 저작권(TTS `msedge-tts` 상업 라이선스 없음·BGM 저작권물·캐릭터 실사 — **개인용은 무관**) ② 백엔드·계정(진도가 localStorage뿐이라 결제 검증 불가, `/api/sets`가 전량 공개) ③ 실제 성적 데이터. 서버 리소스는 **문항 총량 3,160개가 상한이라 1인당 1MB 미만·1,000명 ≈ 375MB**로 무의미한 수준 — 진짜 비용은 오디오 전송량. 접근법은 **로컬 우선 + 동기화**(기록 지점 4곳에 enqueue만 붙이면 됨, 첫 로그인 시 `backup.ts` 스냅샷 업로드로 이전).
- **다음 후보**: 실전 테스트(모의고사) 완성 — `content/mock` 54문항(P5 30·P6 16·P7 8)뿐이고 `mock.ts`·`mock-loader.ts`가 여러 세션째 **미커밋 WIP**(랜딩 3박스 중 하나가 미완) / Part 1(사진) 도입 / VOCA 단어장(memorize SR 엔진 재활용) / 리스닝 세트 제목이 전부 "Question-Response"라 목록 구분 어려움.

### 세션 로그 — 2026-07-24 (39차: 실전 모의고사 완성 — RC 1회분 100문항) ✅ 완료·배포됨
- **맥락**: 38차에서 "랜딩 3박스 중 STEP 3(실전 테스트)가 여러 세션째 미완"으로 남겨둔 것을 마감. 사용자 "이어서 모의고사 마무리하자".
- **미커밋 WIP 상태 정정**: `mock.ts`·`mock-loader.ts`는 이미 커밋됨(38차 디자인 커밋 `d46f20b`에서 `git add -A src/`에 딸려 들어가 커밋·배포까지 완료 — 서버와 일치). 남은 건 `content/mock`(untracked) 콘텐츠뿐이었음.
- **배선 구조(확인)**: `composeMock`의 `dedicatedRc` 경로가 **content/mock 전체를 그대로 RC로 사용**(id 정렬로 Part 5→6→7 순서) → **콘텐츠만 채우면 자동으로 RC 조립**. LC는 은행(content/listening)에서 자동 조립(P2 25·P3 39·P4 30 = 94, Part 1 사진 없음). 화면 `MockRunner`는 `mockCounts`로 총계 동적 계산이라 코드 수정 불필요.
- **Part 7 +46문항** (8 → 54, 실전 토익 구성): 단일 `single-04~10`(7세트×3=21, Notice·광고·문자대화·기사·온라인챗·후기·회람) + 이중 `double-01~02`(2세트×5=10, 채용공고+지원이메일 / 워크숍+등록확인, 교차참조) + 삼중 `triple-01~03`(3세트×5=15, 컨퍼런스/행사장/출장, 세 지문 넘나드는 계산·정보종합 교차참조). 비IT 다양화 + AI·데이터 소재 일부. `mock.ts` `MOCK_TARGET.p7Questions` 24→54(폴백·문서 일관성).
- **검증**: 전수 — 22파일·**RC 100문항(P5 30·P6 16·P7 54)·정답분포 A/B/C/D 각 25 완전균등·마커 100%·id중복0·스키마0**. P7 유형분포 세부26/추론12/주제7/사실확인5/의도3/동의어1(실전 유사). 조립 — id 정렬로 P5→6→7(단일→이중→삼중). `/api/mock` — RC 100 + LC 94 = **194문항**. 화면 — 인트로 "194 총 문항·LC 94·RC 100" 표시, 시작 시 실전 순서(Part 2부터) 진입 확인. `tsc` 0 + 프로덕션 build 통과.
...
### 세션 로그 — 2026-08-19 (42차: 「만점 투자자산운용사」 신규 서비스 — 기출 반복 학습) ✅ 로컬 완료·검증 · ⏸️ 미커밋

- 사용자가 투자자산운용사 자격시험 준비용으로 **기출 100문제(2세트)**를 루트에 MD로 넣고 "토익처럼 풀어서 합격하는 학습법"을 요청. **나머지 900문제는 사용자가 직접 MD로 추가 업로드** 예정.
- **핵심 설계 결정 — 원문 MD를 그대로 읽는다**: 토익·아이엘츠는 ```json 블록이지만, 여기는 사용자가 문서를 폴더에 떨어뜨리는 방식이라 **JSON 변환 단계를 없앴다**(손이 가면 콘텐츠가 안 쌓인다). `content/cim/*.md` 에 문제집 원문(`### Q01.` / `①②③④` / 정답표 / `* **Q01 해설 (정답 ③):**`)을 그대로 두면 파서가 읽는다.
- **학습 모델**: 진도율이 아니라 **간격반복(Leitner, 1·3·7·16·35일)**이 중심. 틀리면 박스 0 + **이번 세션 끝에 즉시 재투입**, 맞히면 박스 +1. 지표도 "몇 문제 풀었다"가 아니라 **"지금 시험 보면 붙는가"** — 합격 기준(총점 70% AND 과목별 40% 과락, 100문항=20/30/50)을 웹검색으로 확인해 반영. 표본 없는 과목은 0점으로 깔지 않고 배점에서 제외 후 100점 환산 + `confidence`(커버리지) 병기.
- **신규 파일**: 도메인 `src/game/cim.ts`(과목 메타·SRS·`buildCimView` 합격 판정·`buildQueue` 4모드) / 로더 `src/lib/cim-loader.ts` / 검증 `scripts/validate-cim.mjs` / API `src/app/api/cim/route.ts`(`?index=1` 경량) / 라우트 `src/app/cim/page.tsx`·`cim/study/page.tsx` / UI `src/components/cim/{CimHome,CimStudy}.tsx`.
- **파서가 잡아낸 원문 결함 2건**(검증기가 실제로 유용했음): ① Set02 Q78 선지 4개가 통째로 인식 안 됨 → 원인은 `
ightarrow`의 백슬래시가 **CR(0x0D)로 깨져** 줄 중간에 박힌 것(`	imes`는 TAB). 제어문자를 원래 이스케이프로 되돌린 뒤 알려진 LaTeX 명령일 때만 복원하는 `restoreEscapes` 추가. ② Q31 해설이 하위 불릿 여러 줄 → 이어지는 줄까지 누적. 최종 **100문항·오류 0·결함 0**, 과목 비중 20/30/50이 실제 시험과 일치.
- **UI**: 팔레트 계약에 **만점 투자자산운용사 = blue** 명문화 + `.btn-blue`·`.text-gradient-blue` 토큰(btn-teal과 같은 문법). 랜딩 서비스 세그먼트에 3번째 📗 투운사 추가(`service.ts`에 `cim`), 3박스는 단계가 아니라 **오늘 할 일**(🔁 오늘의 복습 / 📗 새 문제 / 🎯 합격 판정). 선지 번호는 ①②③④ 글리프가 작은 크기에서 뭉개져 **숫자 배지**로 렌더(🪙 두부 사건과 같은 이유). 학습 기록 키는 `toeic-cim-v1` — 접두사를 유지해야 `backup.ts` 백업/복원에 포함된다.
- **검증**: `validate-cim.mjs` 오류 0 · `tsc --noEmit` 0 · 프로덕션 build 통과(`/cim`·`/cim/study` 생성). 프로덕션 서버 + puppeteer로 **20문항 실제 완주** — 정답 7(box1·내일 예정)/오답 13(box0·즉시), 오답 13개가 같은 세션에 재투입(큐 20→33), 복습 모드에 정확히 13문항, 홈 예상 32점·제1과목 과락 경고, 페이지 에러 0. 데스크탑·모바일 육안 확인.
- **⏸️ 미커밋·미배포**. 루트 원본 `투자자산운용사_핵심기출_Set0{1,2}_50제.md`는 `content/cim/`으로 복사만 하고 루트에 그대로 뒀다(기존 DATA*.md·아이엘츠 원본과 동일 상태 — 처리 방침 미정).
- **후속(같은 세션) — 5문항 = 1스테이지 게임식 구조 ✅ 검증됨**: 사용자 요청("화면에 5개씩 게임식으로, 쪼개서"). 1,000문제를 하나의 긴 목록으로 두면 "어디까지 했더라"가 사라지고 한 번 앉을 때 끊을 지점도 없다 → **`CIM_STAGE_SIZE=5`로 통산 번호 순 분할**(현재 100문항 = **20스테이지**. 사용자는 25판으로 예상했으나 100÷5=20. 1,000문제면 200판으로 자동 확장).
  - `cim.ts`: `buildStages`/`stageShell`(경량 껍데기)/`CimStageRecord`(최고 점수·도전 횟수, **재도전해도 점수가 깎이지 않음**)/`recordStage`/`buildStageViews`/`nextStageNo`. `CimState.stages` 추가하되 **구버전 저장본도 그대로 읽히게** 마이그레이션.
  - `/cim/study?stage=N` → 그 판 5문항을 **순서 그대로**(판마다 같은 순서 = 기억의 좌표). 결과는 게임식 클리어 화면(🏆 만점 / 💪 재도전, 문항별 ○✕ 도트 spring 연출, **다음 스테이지 →** · 이 판 다시 · 목록).
  - `/cim` 홈에 **스테이지 맵**(5·8·10열 반응형 타일: 초록=만점 클리어 / 앰버=도전 중+최고점 / 보라 점=복습 예정 있음 / 파란 링=다음 판) + 「이어서 하기」 배너. 모드 4개는 **복습·틀린 문제 2개(빠른 연습)**로 축소 — 새 문제/전체 순환은 스테이지가 대신. 랜딩 2번째 박스도 "새 문제"→"스테이지"(→ `/cim`)로 경로 일원화.
  - 검증: `tsc` 0 · 프로덕션 build 통과 · puppeteer로 **스테이지1 전부 정답→🏆 클리어(5/5, best 5 저장)**, **스테이지2 전부 오답→💪 재도전(best 0, 오답 재투입)**, 홈 스테이지 맵 색상·클리어 1/20·예상 47점 반영·페이지 에러 0. 데스크탑/모바일 육안 확인.
- **⚠️ 운영 — dev 서버 쓰지 말 것**: `npm run dev`로 띄우면 이 프로젝트 고질병인 `.next` errno -4094(UNKNOWN, `webpack-runtime.js` open 실패)가 **전 라우트 500**으로 재발했고 `/api/cim` 응답이 59초까지 늘어졌다(OneDrive는 원인 아님 — Desktop은 로컬, node 프로세스 3개 동시 점유가 방아쇠). **`npm run build` + `npx next start -p 3000`(프로덕션)으로 테스트하면 한 번도 안 터진다.** 대신 코드 수정 시 재빌드(~2분) 필요.
