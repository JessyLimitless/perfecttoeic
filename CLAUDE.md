...
- **Book 2·3 음원 생성**: `node scripts/tts-warmup.mjs --deck {ootm,present}` (MS Edge TTS `en-US-AriaNeural`). `public/audio/warmup/{ootm,present}/` 각 100 mp3 + `manifest.json`. 이제 3덱 전권 원어민 음원 완비(wmc는 `wmc-full.mp3` 통합본도 있음, ootm/present는 통합본 없음 — UI 미사용이라 생략).
...
- **다음 후보**: 암기(MemorizePlayer)·순서게임에도 음원 연결 / ootm·present 통합본(-full) 생성 / 다른 목소리(성별/영국식) 옵션 / L/C 콘텐츠.
