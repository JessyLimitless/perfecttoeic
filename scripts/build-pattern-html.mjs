#!/usr/bin/env node
// 패턴학습 책자 v2 — content/patterns/*.md(JSON) → 시맨틱 HTML → 인쇄용 PDF.
// 구버전(패턴학습_책자.md + marked)의 "구린" 룩을 폐기하고, 데이터에서 직접 조판한다.
// 표지 · 목차 · 파트 커버(3) · 챕터 간지(15) · 패턴 75 · 예제 260 · 정답 해설.
...
const html = `<!doctype html><html lang="ko"><head><meta charset="utf-8"><style>${CSS}</style></head><body>${bodyHtml}</body></html>`;
